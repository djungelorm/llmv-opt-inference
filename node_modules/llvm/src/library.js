...
var compileExecutable = function(program, pathPrefix) {
  var key = crypto.createHmac('sha256', 'foo').update(compileInstrs(program.instrs)).digest('hex');
  if (key in executableCache) {
    console.log('exe cache hit');
    return executableCache[key];
  }
  var path = pathPrefix + exeId;
  exeId += 1;
  writeFileSync(path+".ll", compileIR(program));
  try {
    execSync('llc -O0 -filetype obj -o '+path+'.o '+path+'.ll', {'stdio': 'ignore'});
  } catch (exn) {
    executableCache[key] = undefined;
    return undefined;
  }
  var exe = path+'.exe';
  execSync('gcc -O0 '+path+'.o -o '+exe);
  executableCache[key] = exe;
  return exe;
};
