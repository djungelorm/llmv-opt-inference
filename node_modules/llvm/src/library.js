...
var compileExecutable = function(program) {
  var key = crypto.createHash('sha256').update(program.inputs+compileInstrs(program)).digest('hex');
  ...
  try {
    child_process.execSync('llc -O0 -filetype obj -o '+path+'.o '+irPath, {'stdio': 'ignore'});
  } catch (exn) {
    executableCache[key] = undefined;
    return undefined;
  }
  child_process.execSync('gcc -O0 utils.o '+path+'.o -o '+exePath);
...
var compileIR = function(program) {
  ...
  return [
    ...
    'declare i8* @toBits(i32)', '',
    '@bits.str = private unnamed_addr constant [4 x i8] c"%s \00", align 1', '',
    '@digit.str = private unnamed_addr constant [4 x i8] c"%d \00", align 1', '',
    'define i32 @main(i32 %argv, i8** %argc) {',
    'body:']
    .concat(inputsExtract)
    .concat(
      '  %r0 = call i32 @program('+inputs+')',
      '  %r0.bits = call i8* (i32) @toBits(i32 %r0)',
      '  call i32 (i8*, ...) @printf(i8* getelementptr inbounds ([4 x i8], [4 x i8]* @digit.str, i32 0, i32 0), i32 %r0)',
      '  call i32 (i8*, ...) @printf(i8* getelementptr inbounds ([4 x i8], [4 x i8]* @bits.str, i32 0, i32 0), i8* %r0.bits)',
      '  ret i32 0',
      '}','')
...
var ssaTransform = function(inputs, instrs) {
  ...
  for (var i = 0; i < instrs.length; i++) {
    ...
    var operands = instr.operands.slice(0);
